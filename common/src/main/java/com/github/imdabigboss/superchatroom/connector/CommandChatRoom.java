...
public class CommandChatRoom {
    ...
    public boolean runCommand(CommandSender sender, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("create")) {
            ...
            if (out == 1)
                ...
                for (ConnectorPlayer player : stringsToPlayers(chatRoom.getRoomPlayers(sender.getName()))) {
            ...
            if (out == 1)
                ...
                for (ConnectorPlayer player : stringsToPlayers(roomPlayers)) {
            ...
            if (Util.platform == ServerPlatform.BUNGEECORD) {
            ...
            if (args[1].equalsIgnoreCase("show")) {
                if (chatRoom.showGeneral.containsKey(sender.getName())) {
                    chatRoom.showGeneral.replace(sender.getName(), true);
                } else {
                    chatRoom.showGeneral.put(sender.getName(), true);
                }
            } else if (args[1].equalsIgnoreCase("hide")) {
                if (chatRoom.showGeneral.containsKey(sender.getName())) {
                    chatRoom.showGeneral.replace(sender.getName(), false);
                } else {
                    chatRoom.showGeneral.put(sender.getName(), false);
                }
            } else {
                SendHelp(sender);
                return true;
    ...
    public List<ConnectorPlayer> stringsToPlayers(List<String> playerNames) {
        List<ConnectorPlayer> players = new ArrayList<>();
        for (String player : playerNames) {
            ConnectorPlayer out = plugin.getPlayer(player);
            if (out != null)
                players.add(out);
        }
        return players;
